...
//        $stmt = $pdo->prepare("SELECT * FROM `".self::DB_TABLE."`" . $sortSQL);
    ...
    public function update() : bool
    {
        ...
        $query = "UPDATE `employee` SET `name` = :name, `surname` = :surname, `job` = :job, `wage` = :wage, `room` = :room, `admin` = :admin WHERE `employee_id` = :employee_id";
        $stmt = PDOProvider::get()->prepare($query);
        return $stmt->execute(['employee_id'=>$this->employee_id, 'name'=>$this->name, 'surname'=>$this->surname, 'job'=>$this->job, 'wage'=>$this->wage, 'room'=>$this->room, 'admin'=>$this->admin]);
    }

    public function updateLog() : bool{
        if (!isset($this->employee_id) || !$this->employee_id)
            throw new Exception("Cannot update model without ID");
        $query = "UPDATE `employee` SET `login` = :login, `password` = :password WHERE `employee_id` = :employee_id";
        $stmt = PDOProvider::get()->prepare($query);
        return $stmt->execute(['employee_id'=>$this->employee_id, 'password'=>$this->password, 'login'=>$this->login]);
    ...
    public function validate(&$errors = [], string $type = "create") : bool
    {
        ...
        if($this->wage < 0)
            $errors['wage'] = 'Wage nemůže být menší než 0';
